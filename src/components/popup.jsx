...
const arrowSX = arrow => {
  ...
  const borderWidth = new Array(4).fill(1).map((_borderSide, borderSideIndex) => borderSideIndex === sideIndex ? '0' : '0.5rem').join(' ')
  const borderColor = new Array(4).fill(1).map((_borderSide, borderSideIndex) => borderSideIndex === (sideIndex + 2) % 4 ? '#2f2f2f' : 'transparent').join(' ')
